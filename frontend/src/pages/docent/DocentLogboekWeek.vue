...
const mentorFeedback = ref(null)
const mentorWeekStatus = ref(null)
const mentorNaam = ref(null)
...
onMounted(async () => {
  ...
  try {
    ...
    mentorFeedback.value = data.mentor_feedback
    mentorWeekStatus.value = data.mentor_week_status
    mentorNaam.value = data.mentor_naam
  } catch (err) {
...
<template>
  <main class="docent-page">
    <header class="topbar">
      ...
      <div class="profile">
        <span>{{ docentInfo?.voornaam }} {{ docentInfo?.achternaam }}</span>
        <button class="logout-btn" @click="logout">Uitloggen</button>
        <div class="avatar">{{ initialen(docentInfo?.voornaam, docentInfo?.achternaam) }}</div>
      </div>
    </header>
    ...
    <div v-else>
      <section class="page-header">
        <button class="back-btn" @click="router.push(`/docent/studenten/${studentId}`)">← Terug naar studentdossier</button>
        <div class="title-row">
          <div>
            <h1>Week {{ weekNummer }} bekijken — {{ student?.voornaam }} {{ student?.achternaam }}</h1>
            <p>Week {{ weekNummer }}</p>
          </div>
          <span class="status-pill" :class="mentorWeekStatus === 'goedgekeurd' ? 'success' : 'warning'">
            {{ mentorStatusLabel(mentorWeekStatus) }}
          </span>
        </div>
      </section>
      ...
      <section v-if="mentorFeedback" class="feedback-section">
        <h2>Wekelijkse feedback van {{ mentorNaam || 'mentor' }}</h2>
        <p class="feedback-tekst">{{ mentorFeedback }}</p>
      </section>
      <section v-else class="feedback-section">
        <h2>Wekelijkse feedback</h2>
        <p class="feedback-leeg">De mentor heeft deze week nog geen feedback gegeven.</p>
      </section>
...
.feedback-section { margin: 10px 64px 26px; background: white; border: 1px solid #e5e7eb; border-radius: 16px; padding: 22px 26px; box-shadow: 0 8px 22px rgba(15,23,42,0.04); }
.feedback-section h2 { margin: 0 0 10px; font-size: 16px; font-weight: 700; }
.feedback-tekst { margin: 0; color: #334155; line-height: 1.6; font-size: 14px; }
.feedback-leeg { margin: 0; color: #94a3b8; font-size: 14px; font-style: italic; }
.actions { margin: 0 64px; padding-bottom: 50px; display: flex; justify-content: flex-start; }
.cancel-btn { border: 1px solid #cbd5e1; background: white; color: #334155; border-radius: 12px; padding: 12px 20px; font-weight: 700; cursor: pointer; font-size: 14px; }
